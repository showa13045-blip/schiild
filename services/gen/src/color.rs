...
/// Exact lookup of the same 256 sRGB transfer values, without approximating Oklab.
pub fn lab_u8(rgb: [u8; 3]) -> Lab {
    static LINEAR: std::sync::OnceLock<[f64; 256]> = std::sync::OnceLock::new();
    let table = LINEAR.get_or_init(|| {
        std::array::from_fn(|n| {
            let v = n as f64 / 255.0;
            if v <= 0.04045 {
                v / 12.92
            } else {
                ((v + 0.055) / 1.055).powf(2.4)
    ...
    linear_lab(rgb.map(|v| table[v as usize]))
}
#[cfg(test)]
mod tests {
    #[test]
    fn byte_lookup_preserves_reference_bits() {
        for r in 0..=255u8 {
            for g in 0..=255u8 {
                let rgb = [r, g, r.wrapping_add(g)];
                assert_eq!(
                    super::lab(rgb.map(f64::from)).map(f64::to_bits),
                    super::lab_u8(rgb).map(f64::to_bits)
                );
            }
        }
    }
