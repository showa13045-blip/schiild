...
M0 PR #1はマージ済み（fc2aa2a）。M1の実装規約は2026-09-08にユーザー承認済み。m1-generationブランチ、PR #2はマージ待ち。M2/M3には着手していない。
...
仕様に算式のない補間・署名・指標等の実装規約はM1-REVIEW.mdへ集約し、ユーザーが採用を承認した。ガターはCLI既定0、本番値は未決として区別する。元のrequirements.md / copy.md / legal.md、共有tokens.ts / copy.tsは変更していない。
